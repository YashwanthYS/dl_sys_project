...
class Flatten(Module):
    def forward(self, X: Tensor) -> Tensor:
        ### BEGIN YOUR SOLUTION
        N = X.shape[0]
        D = int(np.prod(X.shape[1:])) if len(X.shape) > 1 else 1
...
class SoftmaxLoss(Module):
    def forward(self, logits: Tensor, y: Tensor) -> Tensor:
        ### BEGIN YOUR SOLUTION
        N, C = logits.shape
        y_onehot = init.one_hot(C, y, device=logits.device, dtype=logits.dtype)
        correct = ops.summation(logits * y_onehot, axes=(1,))
        log_Z = ops.logsumexp(logits, axes=(1,))
        return ops.summation(log_Z - correct) / N
        ### END YOUR SOLUTION
...
class BatchNorm1d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training:
            ...
            xm = x - ops.broadcast_to(mean, x.shape)
            var = ops.summation(xm * xm, axes=(0,)) / N  
        ...
        inv_std = ops.power_scalar(var + self.eps, -0.5)  
        xhat = (x - ops.broadcast_to(mean, x.shape)) * ops.broadcast_to(inv_std, x.shape)
...
class LayerNorm1d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ### BEGIN YOUR SOLUTION
        N, D = x.shape
        mean = ops.reshape(ops.summation(x, axes=(1,)) / D, (N, 1))
        xm = x - ops.broadcast_to(mean, x.shape)
        var = ops.reshape(ops.summation(xm * xm, axes=(1,)) / D, (N, 1))
        inv_std = ops.power_scalar(var + self.eps, -0.5)
        xhat = xm * ops.broadcast_to(inv_std, x.shape)
...
class Dropout(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if not self.training or self.p == 0.0:
            return x
        keep = 1.0 - self.p
        mask = init.randb(*x.shape, p=keep, device=x.device, dtype=x.dtype)
        return x * (mask / keep)
        ### END YOUR SOLUTION
